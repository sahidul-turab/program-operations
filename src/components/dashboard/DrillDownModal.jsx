...
export function DrillDownModal({ isOpen, onClose, data, pointLabel, mode, dateContext }) {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6">
            ...
            <Card className="relative w-full max-w-2xl max-h-[80vh] flex flex-col bg-[#0f172a] border-slate-800 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                {/* Header */}
                ...
                <div className="flex-grow overflow-y-auto p-6 custom-scrollbar bg-slate-950/20">
                    {data && data.length > 0 ? (
                        <div className="space-y-3">
                            {data.map((item, idx) => (
                                <div key={idx} className="group p-4 bg-slate-900/40 border border-slate-800/60 rounded-xl hover:bg-slate-900/60 hover:border-slate-700/50 transition-all">
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-black text-slate-100 uppercase tracking-tight group-hover:text-shikho-400 transition-colors truncate">
                                        ...
                                        <div className="flex items-center gap-6 text-[10px] font-black uppercase tracking-widest">
                                            ...
                                            <div className="flex items-center gap-2 text-slate-300 bg-slate-800/40 px-3 py-1.5 rounded-lg border border-slate-700/30">
                                                <Clock className="w-3.5 h-3.5 text-shikho-500" />
